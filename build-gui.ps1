# PowerShell script to build chirp-rs with GUI feature
...
Write-Host "Building with GUI feature..."
...
# Clean first since cpp! macro can be onery
cargo clean
...
if ($LASTEXITCODE -eq 0) {
    ...
    # Deploy Qt DLLs to the debug executable
    & C:\Qt\6.10.2\mingw_64\bin\windeployqt.exe --no-translations .\target\debug\chirp-rs.exe

    Write-Host ""
    Write-Host "Done! You can now run: .\target\debug\chirp-rs.exe"
} else {
    Write-Host ""
